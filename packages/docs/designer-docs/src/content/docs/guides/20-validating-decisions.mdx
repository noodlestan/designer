...
...
{
  ...
  name: 'Primary Foregrounds',
  description: 'Green shades for foregrounds',
  usage: { intendedFor: [ 'Typography', 'Foregrounds' ], notFor: [ 'Icons' ] },
  params: {
    start: { '$name': 'Primary Foreground' },
    modifier: {
        "space": "oklch",
        lightness: { type: 'linear', by: 0.05 },
        chroma: { type: 'linear', by: -0.01 }
    },
    steps: 5
  }
}
🐘 4 decisions, 0 errors
```
...
```
🟨 Decision Curcuma Foreground (required) must have required property 'params'
🟨 Decision Primary Foreground (required) must have required property 'params'
🐘 4 decisions, 2 errors
```

## Inspecting and validating decision values

Schema validation does not check for errors such as broken references between decisions because
values are only resolved on-demand.

To show all errors you can the following to your script:
...
🟩 | Curcuma Palette | color-set/anchored | [8] |
