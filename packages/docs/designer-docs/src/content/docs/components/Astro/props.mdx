---
title: Astro component props
slug: components/Astro/Props
description: Common props for Astro design decision components.
sidebar:
    order: 1
    label: Common Props
---

import Banner from '@astrojs/starlight/components/Banner.astro';
import { CardGrid } from '@astrojs/starlight/components';
import { FlexRow, FlexStack, ShowDecision } from '@noodlestan/designer-shows/astro';

import { decisionLoader } from '../../../../decisions';

export const store = await decisionLoader();

## DecisionProps

All components that render a decision, such as the
[&lt;ShowDecisionCard/&gt;](/components/Astro/Cards/show-decision-card/) component,
**require** the following props.

##### ༶ `store`: [StaticDecisionStore](/api/designer-functions/Types/StaticDecisionStore)

The `store` prop provides access to all decision input data so that components can
retrieve decision objects and resolve values.

The simplest way to create a decision store is, as described in the
[Rendering in Astro MDX](/guides/documenting-design-decisions-with-astro-and-mdx)
guide, to create a decision loader and then retrieve the store in your pages.

```mdx
import { loader } from '../decisions';

export const store = await loader();

<ShowDecision store={store} ... />
```

##### ༶ `d`: string | [Decision](/api/designer-decisions/Types/Decision)

A `string` with the name of the decision to visualize or a `Decision` object.

```mdx
<ShowDecision store={store} d="Brand Pink" size="l" />
```

<CardGrid>
    <ShowDecision store={store} d="Brand Pink" size="l" />
</CardGrid>

If the decision does not exist, it renders a notice.

```mdx
<ShowDecision store={store} d="Brand Green" size="m" />
```

<CardGrid>
    <ShowDecision store={store} d="Brand Green" size="m" />
</CardGrid>

## ValueProps

All components that directly render a decision value, such as ...
accept the `viz`, `value` and `size` props.

Note that decision components, such as the
[&lt;ShowDecisionCard/&gt;](/components/Astro/Cards/show-decision-card/) component
mentioned above, delegate to value components. For this reason, they also
accept `viz`, `value` and `size` props.

##### ༶ `viz` (optional = true): boolean

Provide this flag to hide the decision visualization and display only the value.

```mdx
RGB: <ShowDecision store={store} d="Brand Pink" viz={false} />
```

<FlexRow align="center" vpad>
    RGB: <ShowDecision store={store} d="Brand Pink" viz={false} />
</FlexRow>

##### ༶ `value` (optional = true): boolean | string | object

Provide this flag to hide the decision value or customize its format.

```mdx
<ShowDecision store={store} d="Brand Blue" value={false} size="l" />
```

<CardGrid>
    <ShowDecision store={store} d="Brand Blue" value={false} size="l" />
</CardGrid>

With some decision types, you may want to display their value in specific formats. The `value`
prop is passed to the specific decision component to control how the value is displayed.

For instance, the
[&lt;ShowColorValue/&gt;](/components/Astro/Decisions/color#showcolorvaluedecision),
component can display the color value in one or more color spaces.

```mdx
<ShowDecision store={store} d="Action Color" value={['rgb', 'hsl']} size="m" />
```

<CardGrid>
    <ShowDecision store={store} d="Action Color" value={['rgb', 'hsl']} size="m" />
</CardGrid>

##### ༶ `size` (optional = 'auto'): [ShowValueSize](/api/designer-decisions/Types/ShowValueSize/)

Provide a [ShowValueSize](/api/designer-decisions/Types/ShowValueSize/) to control the
visualization size.

```mdx
<ShowDecision store={store} d="Action Color" size={'s'} />
<ShowDecision store={store} d="Space Unit" size={'s'} />

<ShowDecision store={store} d="Action Color" size={'m'} />
<ShowDecision store={store} d="Space Unit" size={'s'} />
```

<FlexStack vpad>
    <CardGrid>
        <ShowDecision store={store} d="Action Color" size={'s'} />
        <ShowDecision store={store} d="Space Unit" size={'s'} />
    </CardGrid>
    <CardGrid>
        <ShowDecision store={store} d="Action Color" size={'m'} />
        <ShowDecision store={store} d="Space Unit" size={'m'} />
    </CardGrid>
</FlexStack>

Refer to [ShowValueSize](/api/designer-decisions/Types/ShowValueSize/) for available sizes.

When no `size` is provided it defaults to `auto`, in which case the visualization
will stretch to use all available space.

```mdx
<ShowDecision store={store} d="Layout Space Scale" />
<ShowDecision store={store} d="Action Color" />
```

Note that this can have different visual results depending on the value being visualized.

For instance, [Space Components](/components/Astro/Decisions/space/) are meant to visualize actual
space decisions and will render

<FlexStack vpad>
    <ShowDecision store={store} d="Layout Space Scale" />
</FlexStack>
<FlexStack vpad>
    <ShowDecision store={store} d="Action Color" value="hsl" />
</FlexStack>
