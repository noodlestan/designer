...
The specific `color-set/anchored` decision model determines how the color scale is generated.
In this case, the model starts with an `anchor` color and applies a `modifier` to create
a number of color `steps` in either/both directions: `up` and `down`
