...
<Aside type="note">
    The common [size](/components/Astro/Props/#-size-auto-showvizsize) prop is not exposed by `
    <ShowDecisionCard />` and the size is managed internally.
</Aside>

In this example we are rendering a [ColorValueDecision](/decision-models/color-value) and at the same time:
