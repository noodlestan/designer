...
-   A list of data sources ([DecisionSource](/api/designer-decisions/Meta/Types/DecisionSource)) from which to load decision inputs ([DecisionInput](/api/designer-decisions/Inputs/Types/DecisionInput/)).
...
The following example configures the decision store with the [schemas](https://github.com/noodlestan/designer/blob/main/packages/libs/designer-schemas/src/constants.ts) for the built-in decision models and adds three data sources: the [demo and sample data](https://github.com/noodlestan/designer/blob/main/packages/libs/designer-decisions/src/meta/data/constants.ts) from `@noodlestan/designer-decisions` and the local `data/` directory.
