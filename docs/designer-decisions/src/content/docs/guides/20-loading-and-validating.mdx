...
const loader = createDecisionLoader([DECISION_SCHEMAS], [DATA_PATH]);
```
...
```js collapse={4-8,24-28} "formatValidationError"
// scripts/validate-decisions.js
import path from 'path';
...
Remove the `store.validationErrors().forEach()` iteration and add these 2 lines to the script:

```diff lang="js" collapse={9-16,26-34} "formatDecisionStatus" "produceDecisions"
// scripts/validate-decisions.js
import path from 'path';
import {
...
```js collapse={2-22,27-36} "filter" "hasErrors"
// scripts/validate-decisions.js
import path from 'path';
import {
    createDecisionLoader,
...
```js collapse={2-23,27-34} "filter" "hasErrors"
// scripts/validate-decisions.js
import path from 'path';
import {
    createDecisionLoader,
...
```js collapse={1-25} "throw" "catch" "exit(1)"
// scripts/validate-decisions.js
import path from 'path';
import {
    createDecisionLoader,
