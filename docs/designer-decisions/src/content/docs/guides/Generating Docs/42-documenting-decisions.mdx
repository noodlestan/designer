...
The minimum required configuration specifies one or more [DecisionSource](/api/designer-decisions/Meta/Types/DecisionSource) from which to load decision data ([InputRecord](/api/designer-decisions/Inputs/Types/InputRecord/)) and the corresponding [SchemaSource](/api/designer-decisions/Meta/Types/SchemaSource) definitions, to load the [schemas](/models/schemas) that validate the input data.
...
<Aside type="tip">
    ...
    guide for more options, including how to store the decision data in another package.
</Aside>
...
As illustrated in the [Capturing Design Decisions](/guides/capturing-design-decisions-in-data) guide, you can find sample data for all [Decision Types](/models/decision-types) that you can easily copy paste into your project.
...
The loader is a function that returns a decision store asynchronously and has to be _awaited on_ before we can access the store to retrieve decisions.
