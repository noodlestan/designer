...
Our [Astro integration](/integrations/Astro) provides:
...
<Aside type="tip">
    If your Astro site in based on [Starlight](), like we do in our [Astro
    Boilerplates](/examples/boilerplates/#astrostarlight), the `designerDecisionsIntegration` needs
    to be added after `starlight()` to make sure our Starlight customizations are applied correctly.
</Aside>
