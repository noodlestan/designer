...
<LayoutGrid>
    <ShowDecision store={store} d="Brand Color Set" layout="column" size="xs" value="oklch" />
    <ShowDecision store={store} d="Foo" layout="column" size="xs" />
</LayoutGrid>
...
-   The [schemas](/models/schemas) for the built-in decision models.
...
export const loader = createDecisionLoader([DECISION_SCHEMAS], [DATA_PATH]);
```
