...
This guide explains how to document design decisions and tokens in your documentation using the [built-in components](/components) provided by the [@noodlestan/designer-shows](/api/designer-shows) package.
...
In this example we are using the [&lt;ShowDecision/&gt;](/components/Astro/ShowDecision/) component to render both a [ColorValueDecision](/decision-models/color-value) and a [ColorSetDecision](/decision-models/color-set).
...
In the [Components reference](/components) you will find a comprehensive set of components for rendering decisions in different scenarios and layouts.

For instance, the [&lt;ShowDecisionCard/&gt;](/components/Astro/ShowDecisionCard/) component composes decision visualization, value, and details in a single layout.
...
All decision components expose [common props](/components/Astro/Props/#valueprops) that provide a high degree of control over how the decisions are rendered:
...
For instance, [Color Decisions](/components/Astro/color) components can display the color value in one or more color spaces.
...
For instance, [Space Decisions](/components/Astro/space) components can represent a space value with a few different shapes.
